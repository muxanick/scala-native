  ...
  private def traverse() = {
    ...
    dir foreach {
      f =>
        ...
        //println(s"traverse:foreach: f.name=${f.name}, dir=${isDirectory}, valid=${validClassFile(f.name)}")
        if (!isDirectory && validClassFile(f.name)) {
          val cr = ClassRep(Some(f), None)
          //println(s"class rep = $cr")
          classBuf += cr
        }
...
extends ClassPath[T] {
  ...
  lazy val classes: IndexedSeq[ClassRepresentation[T]] = {
    ...
    for (e <- entries; c <- e.classes) {
      val name = c.name
      //println(s"classes name=$name")
      if (indices contains name) {
        val idx      = indices(name)
        val existing = cls(idx)
        //println(s"existing=$existing, idx=$idx")
